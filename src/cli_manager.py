...
async def fetch_all_chats():
    ...
    print(f"✓ Fetched total {len(all_chats)} chats")
    return all_chats
...
def print_user_analysis(analysis):
    ...
    print("=" * 80)
    print(f"✗ Deleted Users (accounts deleted): {len(analysis['deleted'])}")
    print(f"✗ No Messages (empty chats): {len(analysis['no_messages'])}")
    print(f"⚠️  Only Incoming (never replied): {len(analysis['only_incoming'])}")
    print(f"🤖 Bots: {len(analysis['bots'])}")
    print(f"🚫 Scam Users (flagged by Telegram): {len(analysis['scam'])}")
    print(f"🚫 Fake Users (flagged by Telegram): {len(analysis['fake'])}")
    print(f"✓ Active Chats (you interacted): {len(analysis['active'])}")
    print(
...
async def export_users_to_json(users):
    ...
    print(
        f"\n📝 To delete users, edit this file and keep only the users you want to DELETE,"
    )
...
async def export_simple_list(items, item_type="items"):
    ...
    print(f"\n✓ Exported {len(items)} {item_type} to {filename}")
    print(
        f"\n📝 To delete items, edit this file and keep only the items you want to DELETE,"
    )
...
async def delete_from_json_file(all_items):
    ...
    print(f"\n✓ Loaded {len(items_to_delete)} items from {filename}")
    ...
    for item in items_to_delete:
        ...
        if item_id in item_map:
            ...
            print(f"✓ [{item_type}] {item.get('title', 'Unknown')} (ID: {item_id})")
        else:
            not_found.append(item)
            print(f"✗ {item.get('title', 'Unknown')} (ID: {item_id}) - Not found")

    if not_found:
        print(f"\n⚠️  Warning: {len(not_found)} items not found")

    if not found_items:
        print("\n✗ No matching items found to delete")
        return

    print(f"\n⚠️  YOU ARE ABOUT TO DELETE {len(found_items)} CHATS!")
    ...
    for i, item in enumerate(found_items, 1):
        try:
            ...
            print(f"[{i}/{len(found_items)}] ✓ Deleted '{item_title}'")
            left_count += 1
        except Exception as e:
            item_title = getattr(
                item, "title", getattr(item, "first_name", f"User_{item.id}")
            ...
            print(f"[{i}/{len(found_items)}] ✗ Error: {e}")

    print(f"\n{'=' * 80}")
    print(f"DELETION SUMMARY")
    print(f"{'=' * 80}")
    print(f"✓ Successfully deleted: {left_count}/{len(found_items)} chats")
...
async def interactive_delete_users(users):
    ...
    for i, user in enumerate(users, 1):
        ...
        # Show warnings/flags
        if is_deleted:
            print("⚠️  DELETED USER (account no longer exists)")
        if is_bot:
            print("🤖 BOT")
        if is_scam:
            print("🚫 SCAM USER (flagged by Telegram)")
        if is_fake:
            print("🚫 FAKE USER (flagged by Telegram)")
        if is_verified:
            print("✓ VERIFIED USER")
        ...
        try:
            ...
            if messages:
                ...
                for msg in messages:
                    direction = "➡️ You" if msg.out else "⬅️ Them"
                    msg_text = (
        ...
        if choice == "q":
            ...
            try:
                ...
                print(f"✓ Deleted chat with '{full_name}'")
                deleted_count += 1
            except Exception as e:
